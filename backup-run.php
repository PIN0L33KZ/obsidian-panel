...
//CLI overwrites
if (PHP_SAPI !== 'cli') {
	error_log("Obsidian Panel Backup: Attempted to run backup-run.php over HTTP!");
	exit("Invalid access type\r\n");
}

if(!isset($argv[2])) {
	error_log("Obsidian Panel Backup: No secret supplied!");
	exit("No secret supplied!\r\n");
}

if(!isset($argv[1])) {
	error_log("Obsidian Panel Backup: No user supplied!");
	exit("No user supplied!\r\n");
}

if(!isset($argv[3])) {
	error_log("Obsidian Panel Backup: No backup auto-delete supplied!");
...
if (!$user) {
	...
	error_log("Obsidian Panel Backup: '" . $user . "' user does not exist!");
	exit('Not Authorized\r\n');
}

//Make sure this page is run via cron and not from URL guessing
if($secret != hash("sha256", $user['pass'])) {
	error_log("Obsidian Panel Backup: Invalid secret!");
	exit('Not Authorized\r\n');
}
...
try {
	...
	error_log("Obsidian Panel Backup: '" . $user . "' Backup Failure!\r\nException : " . $e);
	exit("Exception : " . $e . "\r\n");
}
...
echo "Obsidian Panel Backup Success\r\n";
